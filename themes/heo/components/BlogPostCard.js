  ...
  return (
    <article
      ...
      <div
        data-wow-delay='.2s'
        className={
          'wow fadeInUp border bg-white dark:bg-[#1e1e1e] flex mb-4 flex-col h-[15rem] md:h-52 md:flex-row 2xl:h-96 2xl:flex-col group w-full dark:border-gray-600 hover:border-indigo-600  dark:hover:border-yellow-600 duration-300 transition-colors justify-between overflow-hidden rounded-xl'
        }>
        {/* 图片封面 */}
        {showPageCover && (
        ...
        <div
          className={
            'flex 2xl:p-4 flex-col justify-between h-36 md:h-full 2xl:h-48 w-full md:w-7/12 2xl:w-full'
          }>
          <header>
            {/* 分类 */}
            {post?.category && (
